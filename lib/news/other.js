...
async function itn() {
  ...
  const response = await axios.get(url);
  const $ = cheerio.load(response.data);
  const link = $("div.block-inner > div:nth-child(1) > div > h3 > a").attr(
    "href"
  );
...
async function siyatha() {
  ...
  const $1 = cheerio.load(response1.data);
  const title = $1(
    "div.td_block_wrap.tdb_title.tdi_34.tdb-single-title.td-pb-border-top.td_block_template_1 > div > h1"
  ).text();
  const date = $1(
    "#tdi_29 > div > div.vc_column.tdi_32.wpb_column.vc_column_container.tdc-column.td-pb-span6 > div > div.vc_row_inner.tdi_37.vc_row.vc_inner.wpb_row.td-pb-row > div > div > div > div.td_block_wrap.tdb_single_date.tdi_40.td-pb-border-top.td_block_template_1.tdb-post-meta > div > time"
  ).text();
  const desc = $1(
    "div.td_block_wrap.tdb_single_content.tdi_55.td-pb-border-top.td_block_template_1.td-post-content.tagdiv-type > div > p"
  ).text();
  if (link)
    return {
      title,
...
async function nethnews() {
  ...
  const $ = cheerio.load(response.data);
  const title = $(
    "div > div:nth-child(2) > div > div.col-sm-9 > h3 > a"
  ).text();
  const link = $("div > div:nth-child(2) > div > div.col-sm-9 > h3 > a").attr(
    "href"
  );
...
async function lnw() {
  ...
  const $ = cheerio.load(response.data);
  const title = $(
    "div.td_module_flex_6.td-animation-stack.td-big-grid-flex-post.td-big-grid-flex-post-0.td-cpt-post > div > div.td-module-meta-info > div.tdb-module-title-wrap > h3 > a"
  ).text();
...
async function silumina() {
  ...
  const $ = cheerio.load(response.data);
  const title = $(
    "div.penci-ercol-66.penci-ercol-order-2.penci-sticky-ct.elementor-column.elementor-col-66.elementor-top-column.elementor-element.elementor-element-202cd2f > div > div > div > div > div > div > div > div:nth-child(1) > div > div > div.pcbg-content > div > div > div.pcbg-heading.item-hover > h3 > a"
  )
    .text()
    .trim();
  const date = $(
    "div.penci-ercol-66.penci-ercol-order-2.penci-sticky-ct.elementor-column.elementor-col-66.elementor-top-column.elementor-element.elementor-element-202cd2f > div > div > div > div > div > div > div > div:nth-child(1) > div > div > div.pcbg-content > div > div > div.grid-post-box-meta.pcbg-meta.item-hover > div > span > time"
  ).text();
...
async function dasathalankanews() {
  ...
  const desc = $1(
    "div.td_block_wrap.tdb_single_content.tdi_24.td-pb-border-top.td_block_template_1.td-post-content.tagdiv-type > div > p"
  ).text();
  if (link)
    return {
      title,
...
async function gossiplankanews() {
  ...
  const image = $1(
    "#Blog1 > div.blog-posts.hfeed.container.item-post-wrap > div.blog-post.hentry.item-post > div.post-item-inner > div.post-body.post-content > div.separator > img"
  ).attr("src");
  const desc = $1("p").text();
  if (link)
    return {
...
async function cricbuzz() {
  ...
  const score = $1(
    "div.font-bold.text-center.text-3xl.flex.items-center.flex-row.h-full > div:nth-child(1)"
  ).text();
  if (link)
    return {
      title,
...
async function server() {
  ...
  const desc = $(
    "body > div.outer-wrap > div > main > section > article:nth-child(1) > a > div.post-content > p"
  )
    .text()
    .trim();
